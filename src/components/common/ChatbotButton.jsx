...
const TypingText = ({ text, speed = 30, onComplete }) => {
  ...
  useEffect(() => {
    if (currentIndex < text.length) {
      ...
      // Mark as complete and hide cursor after a delay
      setIsComplete(true);
  ...
  // Cursor blink effect - only when typing or just finished
  useEffect(() => {
    if (!isComplete) {
      const cursorInterval = setInterval(() => {
        setShowCursor(prev => !prev);
      }, 530); // Slightly different timing for better effect
    ...
    <div style={{ position: 'relative' }}>
      ...
      {!isComplete && (
        <span style={{ 
          ...
          marginLeft: '2px',
          animation: currentIndex < text.length ? 'none' : 'blink 1s infinite'
        }}>|</span>
      )}
      <style jsx>{`
        @keyframes blink {
...
const ChatbotButton = () => {
  ...
  // Effect to handle typing animation for new bot messages
  useEffect(() => {
    if (messages.length > 0) {
      ...
      // AND it's not the first message (intro message)
      if (lastIndex > lastProcessedMessageRef.current && 
          lastMessage && 
          lastMessage.type === 'bot' && 
          lastIndex > 0) { // Skip typing animation for the first message (index 0)
        setTypingMessages(prev => {
          const newMap = new Map(prev);
          newMap.set(lastIndex.toString(), true);
          return newMap;
        });
        lastProcessedMessageRef.current = lastIndex;
      } else if (lastIndex === 0 && lastMessage && lastMessage.type === 'bot') {
        // For the first message, just mark it as processed without typing animation
        lastProcessedMessageRef.current = lastIndex;
      }
    }
  }, [messages]);
  ...
  const handleTypingComplete = (messageIndex) => {
    setTypingMessages(prev => {
      const newMap = new Map(prev);
      newMap.delete(messageIndex.toString());
      return newMap;
    });
  };
          ...
          {messages.map((message, index) => (
            message.type === 'bot' ? (
              <BotMessage key={`bot-${index}`}>
                {typingMessages.has(index.toString()) ? (
                  <TypingText 
                    text={message.text} 
                    speed={15}
